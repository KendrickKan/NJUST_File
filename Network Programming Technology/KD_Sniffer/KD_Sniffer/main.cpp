...
int main()
{
	...
	cout << "输入其它数字使用winpcap，解析mac+ip数据\n";
	cout << "请输入>>>";
	int slectMethod = 1;
	cin >> slectMethod;
	...
	if (slectMethod != 1) {
		...
		cout << endl;
		cout << "##########################################" << endl;
		cout << "你现在使用的是winpcap，解析mac+ip数据\n";
		...
		if (pcap_findalldevs_ex(szSource, NULL, &alldevs, errbuf) == 0) {
			d = alldevs;
			while (!(d == NULL)) {
				cout << " 设备 " << i << " 的名称 " << (d->name) << endl;
				cout << " 设备 " << i << " 的描述 " << (d->description) << endl;
				i++;
				d = d->next;
			}
			...
			//}
			cout << "Please chose a device for Sniffer:\n";
			int x;
			cin >> x;
			int i = 0;
			while (i != x) {
